...
template <class T>
class ofxAnimatableObjectMasked : public ofxAnimatableObject<ofFbo>{
public:
    ofxAnimatableObjectMasked() : ofxAnimatableObject<ofFbo>(){
    ...
    /*void draw(){
        ofPushMatrix();
        ofPushStyle();
        ofTranslate(this->position.getCurrentPosition());
        ofRotateX(this->rotation.getCurrentPosition().x);
        ofRotateY(this->rotation.getCurrentPosition().y);
        ofRotateZ(this->rotation.getCurrentPosition().z);
        if(this->size.val()<0) ofScale(0.,0.,0.);
        else ofScale(this->size.val(),this->size.val(),this->size.val());
        ofSetColor(this->color.getCurrentColor());
        fbo.draw(0,0);
        ofPopStyle();
        ofPopMatrix();
    }*/
